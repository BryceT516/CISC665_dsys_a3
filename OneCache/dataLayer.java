		...
		try{
			...
			while(channel.readFlag()){
				try {
					...
					FileServerConnection c = new FileServerConnection(clientSocket, channel);
...
class FileServerConnection extends Thread{
	...
	public void run(){
		...
		} catch(EOFException e) {System.out.println("DL-EOF1: " +e.getMessage());
		} catch(IOException e) {System.out.println("DL-IO1: "+e.getMessage());
